...
//     svc := sts.New(mySession, sdf.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*sdf.Config) *STS {
	c := p.ClientConfig(EndpointsID, cfgs...)
	return newClient(*c.Config, c.Handlers, c.Endpoint, c.SigningRegion, c.SigningName)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg sdf.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *STS {
	svc := &STS{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
