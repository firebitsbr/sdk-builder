...
func TestClientDisableOverrideDefaultHTTPClientTimeout(t *testing.T) {
	svc := ec2metadata.New(unit.Session, sdf.NewConfig().WithEC2MetadataDisableTimeoutOverride(true))
...
func TestClientOverrideDefaultHTTPClientTimeoutRace(t *testing.T) {
	...
	cfg := sdf.NewConfig().WithEndpoint(server.URL)
...
func TestClientOverrideDefaultHTTPClientTimeoutRaceWithTransport(t *testing.T) {
	...
	cfg := sdf.NewConfig().WithEndpoint(server.URL).WithHTTPClient(&http.Client{
...
func runEC2MetadataClients(t *testing.T, cfg *sdf.Config, atOnce int) {
