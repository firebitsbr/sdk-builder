...
func TestChainProviderWithNames(t *testing.T) {
	p := &ChainProvider{
		Providers: []Provider{
			&stubProvider{err: sdferr.New("FirstError", "first provider error", nil)},
			&stubProvider{err: sdferr.New("SecondError", "second provider error", nil)},
			&secondStubProvider{
				creds: Value{
					AccessKeyID:     "AKIF",
...
func TestChainProviderWithNoValidProvider(t *testing.T) {
	errs := []error{
		sdferr.New("FirstError", "first provider error", nil),
		sdferr.New("SecondError", "second provider error", nil),
	}
	p := &ChainProvider{
		Providers: []Provider{
...
func TestChainProviderWithNoValidProviderWithVerboseEnabled(t *testing.T) {
	...
	assert.Equal(t,
		sdferr.NewBatchError("NoCredentialProviders", "no valid providers in chain", errs),
		err,
		"Expect no providers error returned")
}
