...
func TestMissingRequiredParameters(t *testing.T) {
	...
	if e, a := "InvalidParameter", req.Error.(sdferr.Error).Code(); e != a {
		t.Errorf("expect %v, got %v", e, a)
	}
	if e, a := "3 validation error(s) found.", req.Error.(sdferr.Error).Message(); e != a {
		t.Errorf("expect %v, got %v", e, a)
	}

	errs := req.Error.(sdferr.BatchedErrors).OrigErrs()
