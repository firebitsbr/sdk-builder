...
func WithWaiterLogger(logger sdf.Logger) WaiterOption {
	return func(w *Waiter) {
		w.Logger = logger
	}
}
...
type Waiter struct {
	...
	SleepWithContext func(sdf.Context, time.Duration) error
}
...
// Use sdf.BackgroundContext if no context is available.
//
...
func (w Waiter) WaitWithContext(ctx sdf.Context) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if sleepFn := req.Config.SleepDelay; sleepFn != nil {
			...
			if sleepCtxFn == nil {
				sleepCtxFn = sdf.SleepWithContext
			}

			if err := sleepCtxFn(ctx, delay); err != nil {
				return sdferr.New(CanceledErrorCode, "waiter context canceled", err)
			}
		}
	}

	return sdferr.New(WaiterResourceNotReadyErrorCode, "exceeded wait attempts", nil)
}
...
func (a *WaiterAcceptor) match(name string, l sdf.Logger, req *Request, err error) (bool, error) {
	result := false
	var vals []interface{}
	...
	case PathAllWaiterMatch, PathWaiterMatch:
		...
		vals, _ = sdfutil.ValuesAtPath(req.Data, a.Argument)
		if len(vals) == 0 {
			break
		}
		result = true
		for _, val := range vals {
			if !sdfutil.DeepEqual(val, a.Expected) {
				result = false
				break
			}
		}
	...
	case FailureWaiterState:
		...
		return true, sdferr.New(WaiterResourceNotReadyErrorCode,
...
func waiterLogf(logger sdf.Logger, msg string, args ...interface{}) {
	if logger != nil {
