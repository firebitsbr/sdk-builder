...
func unmarshalError(req *request.Request) {
	...
	if err != nil {
		req.Error = sdferr.New("UnmarshaleError", req.HTTPResponse.Status, err)
		return
	}
	if len(bodyBytes) == 0 {
		req.Error = sdferr.NewRequestFailure(
			sdferr.New("UnmarshaleError", req.HTTPResponse.Status, fmt.Errorf("empty body")),
	...
	if err := json.Unmarshal(bodyBytes, &jsonErr); err != nil {
		req.Error = sdferr.New("UnmarshaleError", "JSON unmarshal", err)
		return
	}
	req.Error = sdferr.NewRequestFailure(
...
func TestRequestRecoverRetry5xx(t *testing.T) {
	...
	s := awstesting.NewClient(sdf.NewConfig().WithMaxRetries(10))
...
func TestRequest4xxUnretryable(t *testing.T) {
	...
	aerr := err.(sdferr.RequestFailure)
...
func TestRequestExhaustRetries(t *testing.T) {
	...
	s := awstesting.NewClient(sdf.NewConfig().WithSleepDelay(sleepDelay))
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
	s.Handlers.Send.Clear() // mock sending
...
func TestRequestRecoverExpiredCreds(t *testing.T) {
	...
	s := awstesting.NewClient(&sdf.Config{MaxRetries: sdf.Int(10), Credentials: credentials.NewStaticCredentials("AKID", "SECRET", "")})
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
...
func TestRequestUserAgent(t *testing.T) {
	s := awstesting.NewClient(&sdf.Config{Region: sdf.String("us-east-1")})
	//	s.Handlers.Validate.Clear()
	...
	expectUA := fmt.Sprintf("foo/bar %s/%s (%s; %s; %s)",
		sdf.SDKName, sdf.SDKVersion, runtime.Version(), runtime.GOOS, runtime.GOARCH)
...
func TestRequest_NoBody(t *testing.T) {
	...
	for i, c := range cases {
		...
		s := awstesting.NewClient(&sdf.Config{
			Region:     sdf.String("mock-region"),
			MaxRetries: sdf.Int(0),
			Endpoint:   sdf.String(server.URL),
			DisableSSL: sdf.Bool(true),
		})
		...
		in := struct {
			...
			Bucket: sdf.String("mybucket"), Key: sdf.String("myKey"),
		}
...
func TestIsSerializationErrorRetryable(t *testing.T) {
	testCases := []struct {
		...
		{
			err:      sdferr.New(request.ErrCodeSerialization, "foo error", nil),
			expected: false,
		},
		{
			err:      sdferr.New("ErrFoo", "foo error", nil),
			expected: false,
		},
		{
			err:      nil,
		...
		{
			err:      sdferr.New(request.ErrCodeSerialization, "foo error", stubConnectionResetError),
			expected: true,
		},
	}
...
func TestWithLogLevel(t *testing.T) {
	...
	opt := request.WithLogLevel(sdf.LogDebugWithHTTPBody)
	r.ApplyOptions(opt)

	if !r.Config.LogLevel.Matches(sdf.LogDebugWithHTTPBody) {
...
func TestSerializationErrConnectionReset(t *testing.T) {
	...
	if aerr, ok := err.(sdferr.Error); ok && aerr.Code() != "SerializationError" {
		t.Errorf("Expected 'SerializationError', but received %q", aerr.Code())
	} else if !ok {
		t.Errorf("Expected 'sdferr.Error', but received %v", reflect.TypeOf(err))
...
func TestEnforceShouldRetryCheck(t *testing.T) {
	...
	s := awstesting.NewClient(&sdf.Config{
		...
		Retryer:                 retryer,
		HTTPClient:              client,
		EnforceShouldRetryCheck: sdf.Bool(true),
	})
...
func TestRequest_TemporaryRetry(t *testing.T) {
	...
	svc := awstesting.NewClient(&sdf.Config{
		Region:     unit.Session.Config.Region,
		MaxRetries: sdf.Int(1),
		HTTPClient: client,
		DisableSSL: sdf.Bool(true),
		Endpoint:   sdf.String(server.URL),
	})
	...
	req.Handlers.Unmarshal.PushBack(func(r *request.Request) {
		...
		r.Error = sdferr.New(request.ErrCodeSerialization, "error", err)
	})
...
func TestRequestWillRetry_ByBody(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			WillRetry:  true,
			HTTPMethod: "POST",
			Body:       sdf.ReadSeekCloser(bytes.NewReader([]byte("abc123"))),
		},
		{
			WillRetry:   true,
			HTTPMethod:  "GET",
			Body:        sdf.ReadSeekCloser(bytes.NewBuffer(nil)),
			IsReqNoBody: true,
		},
		{
			WillRetry:   true,
		...
		},
		{
			WillRetry:  false,
			HTTPMethod: "POST",
			Body:       sdf.ReadSeekCloser(bytes.NewBuffer([]byte("abc123"))),
		},
	}
