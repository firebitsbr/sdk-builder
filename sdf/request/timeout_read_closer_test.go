...
func TestAdaptToResponseTimeout(t *testing.T) {
	testCases := []struct {
		...
		{
			childErr: sdferr.New(ErrCodeResponseTimeout, "timeout!", nil),
			r: Request{
				Error: sdferr.New("ErrTest", "FooBar", sdferr.New(ErrCodeResponseTimeout, "timeout!", nil)),
			},
			expectedRootCode: ErrCodeResponseTimeout,
		},
		{
			childErr: sdferr.New(ErrCodeResponseTimeout+"1", "timeout!", nil),
			r: Request{
				Error: sdferr.New("ErrTest", "FooBar", sdferr.New(ErrCodeResponseTimeout+"1", "timeout!", nil)),
			},
			expectedRootCode: "ErrTest",
		},
		{
			r: Request{
				Error: sdferr.New("ErrTest", "FooBar", nil),
			},
			expectedRootCode: "ErrTest",
		},
	}
	...
	for i, c := range testCases {
		...
		if aerr, ok := c.r.Error.(sdferr.Error); !ok {
			t.Errorf("Case %d: Expected 'sdferr', but received %v", i+1, c.r.Error)
