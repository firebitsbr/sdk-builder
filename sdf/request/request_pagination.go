...
func (p *Pagination) Next() bool {
	...
	if p.started {
		for i, intok := range req.Operation.InputTokens {
			sdfutil.SetValueAtPath(req.Params, intok, p.nextTokens[i])
		}
	}
	p.started = true
...
func (r *Request) nextPageTokens() []interface{} {
	...
	if r.Operation.TruncationToken != "" {
		tr, _ := sdfutil.ValuesAtPath(r.Data, r.Operation.TruncationToken)
		if len(tr) == 0 {
			return nil
		}
	...
	for _, outToken := range r.Operation.OutputTokens {
		vs, _ := sdfutil.ValuesAtPath(r.Data, outToken)
		if len(vs) == 0 {
...
func logDeprecatedf(logger sdf.Logger, flag *int32, msg string) {
	if logger == nil {
		return
	}
...
func (r *Request) NextPage() *Request {
	...
	nr := New(r.Config, r.ClientInfo, r.Handlers, r.Retryer, r.Operation, sdfutil.CopyOf(r.Params), data)
	for i, intok := range nr.Operation.InputTokens {
		sdfutil.SetValueAtPath(nr.Params, intok, tokens[i])
	}
	return nr
}
