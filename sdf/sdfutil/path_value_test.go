...
func TestValueAtPathSuccess(t *testing.T) {
	...
	for i, c := range testCases {
		v, err := sdfutil.ValuesAtPath(c.data, c.path)
		if err != nil {
			t.Errorf("case %v, expected no error, %v", i, c.path)
		}
		if e, a := c.expect, v; !sdfutil.DeepEqual(e, a) {
...
func TestSetValueAtPathSuccess(t *testing.T) {
	var s Struct
	sdfutil.SetValueAtPath(&s, "C", "test1")
	sdfutil.SetValueAtPath(&s, "B.B.C", "test2")
	sdfutil.SetValueAtPath(&s, "B.D.C", "test3")
	...
	sdfutil.SetValueAtPath(&s, "B.*.C", "test0")
	...
	var s2 Struct
	sdfutil.SetValueAtPath(&s2, "b.b.c", "test0")
	if e, a := "test0", s2.B.B.C; e != a {
		t.Errorf("expected %v, but received %v", e, a)
	}
	sdfutil.SetValueAtPath(&s2, "A", []Struct{{}})
	if e, a := []Struct{{}}, s2.A; !sdfutil.DeepEqual(e, a) {
		t.Errorf("expected %v, but received %v", e, a)
	}
	...
	s3 := Struct{}
	sdfutil.SetValueAtPath(&s3, "b.b.c", str)
	if e, a := "foo", s3.B.B.C; e != a {
		t.Errorf("expected %v, but received %v", e, a)
	}

	s3 = Struct{B: &Struct{B: &Struct{C: str}}}
	sdfutil.SetValueAtPath(&s3, "b.b.c", nil)
	if e, a := "", s3.B.B.C; e != a {
		t.Errorf("expected %v, but received %v", e, a)
	}
	...
	sdfutil.SetValueAtPath(&s4, "Name", str)
	if e, a := str, *s4.Name; e != a {
		t.Errorf("expected %v, but received %v", e, a)
	}

	s4 = struct{ Name *string }{}
	sdfutil.SetValueAtPath(&s4, "Name", nil)
