...
func buildRequestWithBodyReader(serviceName, region string, body io.Reader) (*http.Request, io.ReadSeeker) {
	...
	if sr, ok := body.(io.ReadSeeker); ok {
		...
		seeker = sdf.ReadSeekCloser(body)
	}
...
func TestAnonymousCredentials(t *testing.T) {
	svc := awstesting.NewClient(&sdf.Config{Credentials: credentials.AnonymousCredentials})
	r := svc.NewRequest(
		&request.Operation{
			Name:       "BatchGetItem",
			HTTPMethod: "POST",
...
func TestResignRequestExpiredCreds(t *testing.T) {
	...
	svc := awstesting.NewClient(&sdf.Config{Credentials: creds})
	r := svc.NewRequest(
		&request.Operation{
			Name:       "BatchGetItem",
			HTTPMethod: "POST",
