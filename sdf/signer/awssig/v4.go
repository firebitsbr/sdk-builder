// Package awssig implements signing for AWS awssig signer
//
// Provides request signing for request that need to be signed with
// AWS awssig Signatures.
//
// Standalone Signer
//
...
//
// AWS awssig signature validation requires that the canonical string's URI path
...
func (v4 Signer) signWithBody(r *http.Request, body io.ReadSeeker, service, region string, exp time.Duration, isPresign bool, signTime time.Time) (http.Header, error) {
	...
	if v4.Debug.Matches(sdf.LogDebugWithSigning) {
...
func (ctx *signingCtx) buildBodyDigest() error {
	...
	if hash == "" {
		if ctx.unsignedPayload || (ctx.isPresign && ctx.ServiceName == "s3") {
			...
			if !sdf.IsReaderSeekable(ctx.Body) {
