...
import (
	...
	"github.com/polefishu/sdk-builder/sdf/awsutil"
)
...
<MockResponse xmlns="http://xmlns.example.com">
	...
	expect := mockOutput{
		String:  sdf.String("string value"),
		Integer: sdf.Int64(123),
		Closed: &mockClosedTags{
			Attr: sdf.String("attr value"),
		},
		Nested: &mockNestedStruct{
			NestedString: sdf.String("nested string value"),
			NestedInt:    sdf.Int64(321),
		},
		List: []*mockListElem{
			{
				String: sdf.String("elem string value"),
				NestedElem: &mockNestedListElem{
					String: sdf.String("nested elem string value"),
					Type:   sdf.String("type"),
				},
			},
		},
	}
...
func TestUnmarshal_UnexpectedEOF(t *testing.T) {
	...
	expect := out
	expect.First = sdf.String("first")
	expect.Second = sdf.String("second")
