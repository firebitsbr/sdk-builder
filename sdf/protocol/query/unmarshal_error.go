...
func UnmarshalError(r *request.Request) {
	...
	if err != nil {
		r.Error = sdferr.New("SerializationError", "failed to read from query HTTP response body", err)
		return
	}
	...
	if decodeErr == nil {
		...
		r.Error = sdferr.NewRequestFailure(
			sdferr.New(resp.Code, resp.Message, nil),
	...
	if unavailErr == nil {
		r.Error = sdferr.NewRequestFailure(
			sdferr.New("ServiceUnavailableException", "service is unavailable", nil),
			r.HTTPResponse.StatusCode,
			r.RequestID,
		)
		return
